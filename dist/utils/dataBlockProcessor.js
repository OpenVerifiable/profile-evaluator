...
function processOneItem(item, jsonData) {
    ...
    if (typeof outValue === "string") {
        ...
        if (!Number.isNaN(num) && outValue.trim() !== "")
            return num;
    }
    ...
    try {
        ...
        if (typeof parsed === "object" && parsed !== null) {
            if (Array.isArray(parsed)) {
                return parsed.map((child) => typeof child === "string" ? processOneItem(child, jsonData) : child);
            }
 ...
 */
export function processOneDataBlock(dataBlock, jsonData) {
    ...
    if (Array.isArray(input.value)) {
        output[input.name] = input.value.map((it) => processOneItem(it, jsonData));
    }
