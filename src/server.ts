...
/**
 * Request type for evaluation.
 * 
 * Either:
 * - profilePath: path to a profile on disk
 * - evalExpression: a JSON Formula expression to run
 * 
 * Plus:
 * - jsonData (required)
 * - optional htmlTemplate path
 * - optional outputFormat ("json" | "yaml" | "html")
 */
...
server.post('/evaluate', async (request: FastifyRequest<{ Body: EvalRequest }>, reply: FastifyReply) => {
  try {
    const {
      jsonData,
      profilePath,
...
// Health check
server.get('/health', async (_, reply: FastifyReply) => reply.send({ status: 'ok' }));
